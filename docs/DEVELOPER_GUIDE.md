...
The one-file build is safe to run while the previous `dist/VoidSmith.exe` is
open. It first stages PyInstaller output under `build/onefile-dist`, then
publishes a new `dist/VoidSmith-<version>.exe` (adding `-rebuildN` if that
versioned artifact already exists). Updating the legacy `VoidSmith.exe` path
is best-effort: a Windows file lock leaves the running copy untouched and does
not fail the newly published build.

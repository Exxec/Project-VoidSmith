...
legal recommendation even when one scripted hullmod remains unknown; the unknown
is reported instead of being treated as a favorable zero.
...
- ordinary supported mechanics remain usable;
- known portions of partially understood scripts are preserved;
- opaque or compiled-only behavior remains unknown;
- fighter-like, module, composite, or otherwise structurally unsupported
  entities can be excluded from ordinary recommendation pools without being
  declared illegal;
- custom combat AI can be detected as evidence without inventing its runtime
  behavior.

This means "partial support" should usually be read as a documented confidence
boundary, not as a parser failure.
...
strikecraft hints. `CombatEntityKind` says **what the entity is**;
`DeploymentModel` records **how much is known about how it enters combat**.
Fighter-sized geometry alone does not prove wing membership, and an entity label
does not by itself imply a tactical role.
...
- **battlefield function**: what job the hull appears suited to perform;
- **engagement position**: where it appears mechanically comfortable fighting;
- **tactical style**: the shape of its supported engagement behavior;
- **tempo**: whether its parsed mechanics favor sustained or more burst-oriented
  pressure;
- **commitment**: how strongly the visible mechanics suggest committing to an
  engagement;
- **fleet dependence**: how much the available evidence supports independent
  versus formation/support-oriented use.

The profile is multi-valued rather than a single hard "class." It only uses
parsed defenses, flux, mobility, mounts/arcs, bays, and observed

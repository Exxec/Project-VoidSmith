...
The implemented pack field is `approved_equipment`: a resolved entry becomes
`APPROVED` only for its target faction. Strict Faction generation includes
approved weapons, fighters, and hullmods in its allowed pool; Faction+ marks
them preferred. `APPROVED` remains subject to every normal legality gate.
